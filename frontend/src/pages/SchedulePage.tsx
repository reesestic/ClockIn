...
const STYLE_PRESETS: Record<string, { deadline: string; importance: string; time: string }> = {
    Balanced:          { deadline: "none", importance: "none", time: "none" },
    "Deadline First":  { deadline: "desc", importance: "none", time: "none" },
    "Importance First":{ deadline: "none", importance: "desc", time: "none" },
    "Morning Heavy":   { deadline: "none", importance: "none", time: "desc" },
};
...
export default function SchedulePage({ onClose }: { onClose?: () => void }) {
    ...
    useEffect(() => {
        getTasksForUser().then(setAllTasks).catch(console.error);
    }, []);
    ...
    const unselectedTasks = allTasks.filter(t => !selectedIds.has(t.id));
    ...
    async function handleGenerate() {
        ...
        setLoading(true);
        try {
            const filters = { ...(STYLE_PRESETS[style] as any), allowed_days: Array.from(activeDays) };
